export const MainNavigation = [
  {
    name: '/svg/store.svg',
    link: '/catalog',
    title: 'Каталог'
  },
  {
    name: '/svg/cart.svg',
    link: '/cart',
    title: 'Корзина'
  }
]

export const HeaderNavigation = [
  // {
  //     name: 'Профиль',
  //     link: '/profile'
  // },
  {
    name: 'Домашняя страница',
    link: '/home'
  },
  {
    name: 'Каталог',
    link: '/catalog'
  },
  {
    name: 'Корзина',
    link: '/cart'
  }
]

export const introduction = [
  {
    type: 'сyberware',
    title: 'Киберимпланты',
    desc: 'Киберимпланты, аугментации, хром — класс специализированной кибернетической технологии, которая помещается в тело для выполнения определенной функции. Тип биомеханических устройств, предназначенных для замены поврежденных и больных частей тела, улучшения его возможностей и силы, а также в некоторых случаях продления жизни.',
    img: '/img/сyberware.png'
  },
  {
    type: 'ripperdoc',
    title: 'Риперы',
    desc: 'Риперы — специалисты, занимающиеся установкой кибернетических имплантов и проведением пластических операций. Они представляют собой смесь хирургов, медиков, татуировщиков и терапевтов, которые могут модифицировать тело человека с помощью различных кибернетических устройств.',
    img: '/img/ripperdoc.png'
  },
  {
    type: 'сyberpsychosis',
    title: 'Киберпсихоз',
    desc: 'Киберпсихоз — диссоциативное расстройство личности, возникающее, когда человек с уже имеющимися психическими отклонениями ставит себе столько киберимплантов, что начинает считать себя или окружающих не полноценными разумными существами, а набором заменимых деталей. Избыток имлпантов может навредить вашему телу и рассудку, будьте осторожны.',
    img: '/img/сyberpsychosis.png'
  }
]

export const menu = [
  {
    img: '1',
    description:
      'Профиль',
  },
  {
    img: '2',
    description:
      'Домашняя страница',
  },
  {
    img: '3',
    description:
      'Каталог',
  },
  {
    img: '4',
    description:
      'Корзина',
  },
]

export const CellsFirst = [
  {
    type: 'frontal_cortex',
    text: 'Лобная доля',
    link: '/catalog/frontal_cortex'
  },
  {
    type: 'arms',
    text: 'Руки',
    link: '/catalog/arms'
  },
  {
    type: 'skeleton',
    text: 'Скелет',
    link: '/catalog/skeleton'
  },
  {
    type: 'nervous_system',
    text: 'Нервная система',
    link: '/catalog/nervous_system'
  },
  {
    type: 'integumentary_system',
    text: 'Кожа',
    link: '/catalog/integumentary_system'
  }
]

export const CellsSecond = [
  {
    type: 'operating_system',
    text: 'Операционная система',
    link: '/catalog/operating_system'
  },
  {
    type: 'face',
    text: 'Лицо',
    link: '/catalog/face'
  },
  {
    type: 'hands',
    text: 'Ладони',
    link: '/catalog/hands'
  },
  {
    type: 'circulatory_system',
    text: 'Кровеносная система',
    link: '/catalog/circulatory_system'
  },
  {
    type: 'legs',
    text: 'Ноги',
    link: '/catalog/legs'
  }
]

export const Offer = [
  {
    title: 'Акция',
    link: '/catalog',
    img: '/img/offer/offer_1.jpg',
    desc: 'При покупке от 3-х имплантов - установка от рипера бесплатно'
  },
  {
    title: 'Не упустите возможность',
    link: '/catalog/frontal_cortex',
    img: '/img/offer/offer_2.jpg',
    desc: 'Заказывайте импланты для лобной доли и получите разведчика xN2 в подарок'
  },
  {
    title: 'Мы открылись',
    link: 'https://yandex.ru/maps/-/CDr7i81s',
    img: '/img/offer/offer_3.jpg',
    desc: 'Пункт выдачи имплантов теперь и в Твери'
  }
]
